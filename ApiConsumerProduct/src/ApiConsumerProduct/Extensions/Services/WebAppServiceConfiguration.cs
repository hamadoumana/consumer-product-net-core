...
using ApiConsumerProduct.Middleware;
...
public static class WebAppServiceConfiguration
{
    ...
    {
        builder.Services.Configure<ApiConsumerProductOptions>(builder.Configuration.GetSection(ApiConsumerProductOptions.SectionName));
        ...
        builder.Services.AddCorsService("ApiConsumerProductCorsPolicy", builder.Environment);
        builder.OpenTelemetryRegistration(builder.Configuration, "ApiConsumerProduct");
        ...
        // registers all services that inherit from your base service interface - IApiConsumerProductScopedService
    ...
    private static void AddBoundaryServices(this IServiceCollection services, params Assembly[] assemblies)
    {
        ...
        foreach (var assembly in assemblies)
        {
            var rules = assembly.GetTypes()
                .Where(x => !x.IsAbstract && x.IsClass && x.GetInterface(nameof(IApiConsumerProductScopedService)) == typeof(IApiConsumerProductScopedService));
